...
class ClassicTableViewController: UITableViewController, RefreshViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        V2netTool.shared.getClassicMovieList(success: {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let classic = classicData[indexPath.row]

        // Configure the cell...
        cell.title.text = classic.title
        cell.director.text = classic.director
        cell.releaseTime.text = classic.releaseTime
        cell.decade.text = classic.decade
        cell.specise.text = classic.specise
        
        let speciesWidthConstranit = cell.specise.constraints.filter{
            return $0.identifier == "speciesWidth"
            }.first
        speciesWidthConstranit?.constant = v2tools.getTextWidth(string: classic.specise, font: cell.specise.font) + 5.0
        let directorWidthConstraint = cell.director.constraints.filter{
            return $0.identifier == "directorWidth"
            }.first
        directorWidthConstraint?.constant = v2tools.getTextWidth(string: classic.director, font: cell.director.font) + 5.0
        let releaseTimeWidthConstraint = cell.releaseTime.constraints.filter{
            return $0.identifier == "releaseTimeWidth"
            }.first
        releaseTimeWidthConstraint?.constant = v2tools.getTextWidth(string: classic.releaseTime, font: cell.releaseTime.font) + 5.0
